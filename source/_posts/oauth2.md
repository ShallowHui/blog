...
一般是通过`GET`请求去访问授权服务器的授权端点，并以`application/x-www-form-urlencoded`的方式携带以下参数：
...
比如，如何让用户的代理跳转到这个地址？具体对于一个第三方Web应用来说，使用微信进行登录，就要让浏览器界面跳转到微信自身的授权网站，这个网站上会显示一个二维码，用户使用手机微信扫码后就授权登录了。一般的，可以这样实现：浏览器向后端接口发起请求，后端返回一个重定向响应，重定向的目标地址就是授权服务器的授权端点并带上参数，这样用户的浏览器就会跳转到授权页面了。

客户端通过重定向，让用户代理跳转到授权服务器的授权端点。一般的，授权服务器会先对用户进行一个认证，认证成功后，再显示是否允许对客户端的授权，以及确认授予客户端哪些权限，这个权限范围对应的就是`scope`参数。用户同意之后，授权服务器也会返回给用户代理一个重定向响应，目标地址就是`redirect_url`参数指定的地址。