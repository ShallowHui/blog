...
当用户希望在第三方应用上，使用自己其它平台的信息时，比如想要在第三方应用上使用微信进行登录，就需要由第三方应用也就是客户端，向授权服务器发起一个授权请求。

一般是通过GET请求去访问授权服务器的授权端点，并以`application/x-www-form-urlencoded`的方式携带以下参数：

| | 参数名 | 描述 |
| :------: | :------: | :------: |
| 必须 | response_type | 响应类型，这里必须为`code` |
| 必须 | client_id | 上面注册时获取到的客户端id |
| 可选 | redirect_uri | 回调地址，如果为空，则使用注册时提交的回调地址 |
| 可选 | scope | 访问请求的范围 |
...
```text
GET https://authorization_server/authorize?response_type=code&client_id=s6BhdRkqt3&state=xyz&redirect_uri=https%3A%2F%2Fclient%2Eexample%2Ecom%2Fcb
```

值得注意的是，这里说得都是OAUth 2.0的规范步骤，至于具体的实现细节需要结合实际情况自己考虑。

比如，如何让用户的代理跳转到这个地址？具体对于一个第三方Web应用来说，使用微信进行登录，就要让浏览器界面跳转到微信自身的授权网站，这个网站上会显示一个二维码，用户使用手机微信扫码后就授权登录了。一般的，可以这样实现：前端向后端接口发起请求，后端返回一个重定向响应，重定向的目标地址就是授权服务器的授权端点并带上参数，这样用户的浏览器就会跳转到授权页面了。

客户端通过重定向，让用户代理跳转到授权服务器的授权端点。一般的，授权服务器会先对用户进行一个认证，认证成功后，再显示是否允许对客户端的授权，以及确认授予客户端哪些权限，这对应的就是`scope`参数。用户同意之后，授权服务器就会返回给用户代理一个重定向响应，目标地址就是`redirect_url`参数。